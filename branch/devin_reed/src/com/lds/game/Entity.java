...
public abstract class Entity //all objects in game, including enemies, character, pickup objects, physics objects, and static objects are derived from this class 
{
	...
	public void initialize (float _size, float _xPos, float _yPos, float _angle)
	{
		initialize(_size, _xPos, _yPos, _angle, 0.0f, 0.0f);
	}
	
	public void initialize (float _size, float _xPos, float _yPos)
	{
		initialize(_size, _xPos, _yPos, 0.0f, 0.0f, 0.0f);
	}
	...
	{
		if (angle == 0 && ent.angle == 0)
		{
			if ((float)(Math.abs(xPos - ent.xPos)) <= size && (float)(Math.abs(yPos - ent.yPos)) <= size)
				return true;
		}
		else
		{
